...
import com.zmm.tmsystem.ui.widget.chat.browser.BitmapLoader;
import com.zmm.tmsystem.ui.widget.chat.browser.ImgBrowserViewPager;
import com.zmm.tmsystem.ui.widget.chat.browser.NativeImageLoader;
import com.zmm.tmsystem.ui.widget.chat.browser.PhotoView;
...
public class BrowserViewPagerActivity extends BaseActivity {
    ...
    TextView mNumberTv;
    @BindView(R.id.pick_picture_send_btn)
    Button mSendBtn;
    ...
    Button mLoadBtn;
    @BindView(R.id.origin_picture_cb)
    CheckBox mOriginPictureCb;
    @BindView(R.id.total_size_tv)
    TextView mTotalSizeTv;
    @BindView(R.id.choose_tv)
    TextView chooseTv;
    @BindView(R.id.picture_selected_cb)
    CheckBox mPictureSelectedCb;
    @BindView(R.id.check_box_rl)
    RelativeLayout checkBoxRl;
    @BindView(R.id.image_browser_view)
    RelativeLayout imageBrowserView;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        String targetAppKey = intent.getStringExtra(Constant.TARGET_APP_KEY);
        ...
        System.out.println("returnBtn = "+returnBtn);
        System.out.println("mSendBtn = "+mSendBtn);
        System.out.println("mLoadBtn = "+mLoadBtn);
...
//        }
    ...
    private void initImgPathList() {
        mMsgIdList = this.getIntent().getIntegerArrayListExtra(Constant.MsgIDs);
        Message msg;
        ImageContent ic;
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.return_btn:
                    ...
                    setResult(Constant.RESULT_CODE_SELECT_PICTURE, intent);
                    finish();
                    break;
                ...
                imgContent.downloadOriginImage(mMsg, new DownloadCompletionCallback() {
                    ...
                    public void onComplete(int status, String desc, File file) {
                        ...
                        if (status == 0) {
                            ...
                            bundle.putInt(Constant.POSITION, mViewPager.getCurrentItem());
                            msg.setData(bundle);
                            msg.sendToTarget();
                        } else {
                            if (mProgressDialog != null) {
            ...
            if (activity != null) {
                switch (msg.what) {
                    case DOWNLOAD_ORIGIN_IMAGE_SUCCEED:
                        ...
                        activity.mPathList.set(bundle.getInt(Constant.POSITION), bundle.getString("path"));
                    ...
                    case SEND_PICTURE:
                        Intent intent = new Intent();
                        intent.putExtra(Constant.MsgIDs, activity.mMsgIds);
                        activity.setResult(Constant.RESULT_CODE_BROWSER_PICTURE, intent);
                        activity.finish();
                        break;
                    //显示下载原图进度
